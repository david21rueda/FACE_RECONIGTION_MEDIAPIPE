    ...
    if cap != None:
        ...
        if ret == True:
            ...
            if PredictionEmotion != "No hay rostro":
                f_show = cv2.resize(f_show, (459, 480))
                f_pl = cv2.resize(f_pl, (395, 480))
                f_con = cv2.hconcat([f_show, f_pl])
...
lblPrediction.grid(column=0, row=4, columnspan=2)
